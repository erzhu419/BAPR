...
class ContextNetwork(nn.Module):
    ...
    def forward(self, state_with_emb):
        # normalize output to avoid large ep_tensor blowing up distances
...
class SoftQNetwork(VectorizedCritic):
    def __init__(self, state_dim, action_dim, hidden_dim, embedding_layer, ep_dim=4, ensemble_size=10):
    ...
    def forward(self, state, action, ep_tensor):
...
class PolicyNetwork(nn.Module):
    def __init__(self, num_inputs, num_actions, hidden_size, embedding_layer, context_net, ep_dim=4, action_range=1., init_w=3e-3, log_std_min=-20, log_std_max=2):
    ...
    def forward(self, state):
        ...
        # During warmup, zero out ep_tensor so policy behaves as pure RE-SAC
        if getattr(self, '_warmup_active', False):
            ep_tensor = torch.zeros_like(ep_tensor)
        state_with_embeddings_ep = torch.cat([state_with_embeddings, ep_tensor], dim=1)

        x = F.relu(self.linear1(state_with_embeddings_ep))
...
class SAC_Trainer():
    def __init__(self, env, replay_buffer, hidden_dim, action_range, ensemble_size=10):
        ...
        self.policy_net = PolicyNetwork(state_dim, action_dim, hidden_dim, embedding_layer, self.context_net, ep_dim=args.ep_dim, action_range=action_range).to(device)
    ...
    # Q loss computation
    def compute_q_loss(self, state, action, reward, next_state, done, new_next_action, next_log_prob, reg_norm, gamma, ep_tensor, next_ep_tensor):
        predicted_q_value = self.soft_q_net(state, action, ep_tensor)  # shape: [ensemble_size, batch, 1]
        # with torch.no_grad():
        target_q_next = self.target_soft_q_net(next_state, new_next_action, next_ep_tensor)  # shape: [ensemble_size, batch, 1]
    ...
    def compute_policy_loss(self, state, action, new_action, log_prob, reg_norm, ep_tensor):
    ...
    def compute_rmdm_loss(self, ep_tensor, task_ids):
        ...
        cons_loss = sum(consistency_losses) / len(consistency_losses) if len(consistency_losses) > 0 else 0.0
    ...
    def update(self, batch_size, training_steps, reward_scale=10., auto_entropy=True, target_entropy=-2, gamma=0.99, soft_tau=1e-2, current_episode=0):
        ...
        # GET EMBEDDINGS to compute ep_tensor BEFORE RL losses
        ...
        # Update ContextNetwork via RMDM (always, even during warmup)
        ...
        # Also set the flag on policy_net so forward()/evaluate() respect warmup
        ...
        q_value_loss, predicted_q_value, ood_loss = self.compute_q_loss(state, action, reward, next_state, done, new_next_action, next_log_prob, reg_norm, gamma, ep_tensor, next_ep_tensor)
...
env = env_bus(path, debug=debug, route_sigma=args.route_sigma,
              ...
              mode_switch_interval=(args.mode_switch_min, args.mode_switch_max))
env.reset()
...
if __name__ == '__main__':
    if args.train:
        ...
        for eps in range(args.max_episodes):
            ...
            # Set warmup flag for rollout actions
            ...
            while not done:
                ...
                if len(replay_buffer) > args.batch_size and len(replay_buffer) % args.training_freq == 0 and step_trained != step:
                    ...
                    for i in range(update_itr):
                        _ = sac_trainer.update(args.batch_size, training_steps, reward_scale=10., auto_entropy=args.auto_entropy, target_entropy=-1. * action_dim, current_episode=eps)
                        training_steps += 1
            ...
            print(
                ...
                f"{f' | Mode: {env.current_mode_name} | Switches: {env.mode_switch_count}' if args.enable_mode_switch else ''}"
                f"{' | [WARMUP]' if eps < args.context_warmup_episodes else ' | [ESCP ACTIVE]'}")
