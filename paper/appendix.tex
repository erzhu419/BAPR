% ARCHIVED PRE-V32 APPENDIX.
% This file analyzes the retired BOCD/LCB/RMDM algorithm and is intentionally
% excluded from the V32 manuscript. Current proofs are in appendix_v32.tex.
\newpage
\appendix
\onecolumn
\begin{center}
...
We analyze how BAPR's piecewise-robust design yields a polynomial recovery-count bound for the abstract operator under the assumptions below, in contrast to the exponential barrier faced by direct risk-sensitive optimization. Unlike the informal ``regret'' framing common in deep RL theory papers, we state a \emph{convergence rate} result---measuring how fast $\|Q_t - Q^*\|_\infty$ decays---which is the natural theoretical tool for contraction-based algorithms. The algebraic components of the bound below correspond to machine-verified Lean~4 theorems.
...
\end{equation}
BAPR sidesteps this specific risk-sensitive lower-bound setting at the operator level through the same mechanism as RE-SAC~\cite{Zhang2026RESAC}: replacing tail-distribution estimation with frozen structural penalties, which converts the abstract backup to a standard shifted-reward MDP with $\gamma$-contraction.

\subsection{Formal error budget}

The BAPR convergence-rate argument decomposes into five components, each tied to a machine-verified algebraic bound:
...
\begin{theorem}[Piecewise Q-value convergence rate --- formal]
    ...
    \end{equation}
    which is polynomial in the listed parameters under the stated assumptions (with no exponential dependence on $H$ or $\beta$ in this operator-level bound).
\end{theorem}
\begin{proof}
...
\end{itemize}
Each displayed inequality above corresponds to a machine-verified algebraic theorem; the surrounding modeling assumptions are stated explicitly in the theorem. \qed
\end{proof}

\begin{remark}[Why convergence rate, not regret]
    We deliberately state our result as a \emph{convergence rate} ($\|Q_t - Q^*\|_\infty$ decay) rather than a \emph{regret bound} ($\sum_t V^* - V^{\pi_t}$ accumulation). A formal regret bound would require exploration-exploitation analysis (how quickly the policy visits informative states), which SAC---like all off-policy deep RL methods---does not provide. For the abstract soft operator, the convergence rate can be converted into standard policy-quality bounds such as $\|V^{\pi_t} - V^*\| \leq 2\|Q_t - Q^*\|/(1-\gamma)$ under the usual value-difference assumptions.
\end{remark}

\begin{remark}[Comparison to static robust methods]
    A static robust method (e.g., Robust MDP with a fixed uncertainty set calibrated for the worst-case regime) can incur a persistent suboptimality of $\Omega(\Delta)$ per step in favorable regimes. Over $T$ steps with $N \ll T$ switches, the BAPR recovery-count expression $N \cdot O(\log(1/\delta)/\log L + \log(\Delta_R/\varepsilon)/(1-\gamma))$ is smaller than the static cost $\Omega(\Delta \cdot T)$ in regimes where the assumptions hold and switches are sparse.
\end{remark}
...
\textbf{Base case:} The first segment $[t_0, t_1)$ has no prior contamination. The BAPR operator with uniform initial belief is a $\gamma$-contraction (Theorem~\ref{thm:bapr_full}), and the combined approximation-stochastic bound (Theorem~\ref{thm:combined}) gives the stated recovery bound.
...
\begin{itemize}
    ...
    \item The BOCD posterior shifts to higher expected run-length $\bar h$ within $n_\delta = O(\log(1/\delta)/\log L)$ steps (Theorem~\ref{thm:delay}); this raises $\bar h/(H{-}1)$ above the EMA baseline and triggers $\lambda_w > 0$.
    \item $\beta_{\text{eff}}$ becomes more conservative, providing pessimistic bias during the transient.
    ...
    \item By Assumption~\ref{assump:metastable}, there are at least $1/(1-\gamma)$ steps remaining before the next switch, sufficient for the stated contraction-recovery phase. \qedhere
\end{itemize}
\end{proof}
...
\end{enumerate}
The Lean proof makes each of these requirements explicit and machine-verified. Violating any one of them breaks this proof route; the counterexample demonstrates one concrete contraction failure for condition (3).
...
\noindent This corollary is the critical bridge between RE-SAC (which provides the $\kappa$ surprise signal) and BAPR (which uses it for belief updating): at the abstract-operator level, each valid frozen posterior snapshot again satisfies the contraction theorem's assumptions.


\section{Counterexample: Q-dependent belief weights break contraction}
\label{app:counterproof}

Theorem~\ref{thm:bapr_full} requires the belief $\rho$ to be \emph{frozen} during the Bellman backup. This section shows why that requirement is load-bearing for this proof class: allowing $\rho$ to depend on the Q-function being updated can destroy contractivity. The results are machine-verified in \texttt{BAPR-Counterproof.lean}.
...
\textbf{The critical threshold.} The contraction factor in this constructed operator is $\gamma + \lambda\Delta$. For typical RL parameters ($\gamma = 0.99$), any $\lambda\Delta \geq 0.01$ breaks contraction in the construction. Thus even small Q-sensitivity can be problematic when the mode reward gap is large.
...
\noindent\textbf{Parallel structure with RE-SAC.} The BAPR counterexample is structurally analogous to RE-SAC's counterexample (\texttt{RESAC-Counterproof.lean}), but with a different amplification mechanism: in RE-SAC, Q-dependent \emph{aleatoric penalties} cause expansion; in BAPR, Q-dependent \emph{belief weights} cause expansion. Together, they show concrete ways that non-frozen adaptive quantities can invalidate the contraction argument. The fact that two independent amplification channels exist underscores the importance of the frozen-parameter design principle across robust RL architectures.
...
Following the same analysis as RE-SAC~\cite{Zhang2026RESAC}:
\begin{equation}
    Q^*_{\text{BAPR}}(s,a) = Q^*(s,a) - \frac{\gamma \cdot \Delta_{\text{penalty}}}{1-\gamma},
\end{equation}
where $\Delta_{\text{penalty}}$ includes the belief-weighted contribution. With the experimental configuration ($\gamma = 0.99$, $\lambda_w \leq 0.4$ empirically, $c_{\text{penalty}} = 0.5$, $|\beta_{\text{base}}| = 2$), the maximum additional depression from the adaptive penalty is bounded by $\gamma \cdot 0.5 \cdot \bar{\sigma}_{\text{ens}}/(1-\gamma) \approx 50 \cdot \bar{\sigma}_{\text{ens}}$, which remains small relative to the typical Q-value scale in our experiments.
...
\noindent This machine-verified result guarantees a monotonicity property of the surrogate LCB objective: false positive change-point detections can only make $\beta_{\text{eff}}$ more conservative, not more optimistic than the RE-SAC baseline. The ``Bayesian Amnesia'' name reflects this one-directional modulation in the objective.
...
\begin{proposition}[Context embedding equivalence --- Lean: \texttt{context\_embedding\_equiv}]
    ...
    \end{equation}
    When the context embedding $\phi: \mathcal{S} \to \mathbb{R}^{d_e}$ is injective (different modes produce distinct embeddings) and the critic class is sufficiently expressive, a shared critic $Q_\phi(s \oplus \phi(s), a)$ can represent the collection of mode-conditional Q-functions. In this idealized representational case, the scalar approximation in Remark~\ref{rem:tractable} need not introduce additional representation error beyond the stated approximation terms.
\end{proposition}

\noindent This narrows the theory-practice gap: while the abstract operator assumes $M$ independent critics, a shared critic with a sufficiently expressive context embedding can recover the same function class in principle. The remaining approximation error depends on the context module's ability to separate modes---which is what the RMDM loss (\S\ref{sec:context}) is designed to encourage.
...
The detection delay bound (Theorem~\ref{thm:delay}) requires $L > 1$. In environments where regime changes are very subtle (small $\Delta$), the detection delay grows as $O(1/\log L)$, potentially exceeding the metastable period. In this regime, the intended fallback behavior is:
\begin{itemize}
    \item \textbf{Detection failure ($L \approx 1$):} If the change is too subtle to detect, $\lambda_w$ does not spike, and $\beta_{\text{eff}} \approx \beta_{\text{base}}$. BAPR behaves close to RE-SAC in the LCB objective, losing the \emph{adaptive} conservatism benefit but retaining the \emph{static} robustness mechanism.
    ...
    \item \textbf{False-positive effect:} By construction ($\beta_{\text{eff}} \leq \beta_{\text{base}}$), false positive detections can only increase conservatism in the surrogate objective, never decrease it. The expected cost is temporary over-conservatism.
\end{itemize}
...
The contraction proof uses frozen quantities at \emph{three} levels: (i) the aleatoric penalty $\kappa$ (from RE-SAC), (ii) the epistemic penalty $\Gamma_{\text{epi}}$ (from RE-SAC), and (iii) the belief distribution $\rho$ (new in BAPR). Counterexamples show that Q-dependent variants can fail. This multi-level frozen-parameter design is the key architectural constraint that distinguishes BAPR from naive adaptive approaches.
...
The function-approximation and stochastic tracking bounds from \texttt{ApproxContraction.lean} apply to the abstract BAPR operator because they are operator-agnostic. The projected contraction, approximation error bound, and stochastic tracking bound therefore carry over to that operator-level setting, while the practical deep-RL implementation still depends on the stated approximation assumptions.
...
In a stationary environment with a single mode ($|\mathcal{M}| = 1$), the belief is trivially $\rho(m_0) = 1$, and the abstract BAPR operator reduces to the RE-SAC operator. The adaptive $\beta_{\text{eff}}$ also approaches $\beta_{\text{base}}$ when the BOCD surprise signal remains at its steady-state baseline.

\subsection{BAPR as an extension of ESCP}

ESCP~\cite{Luo2022ESCP} provides context-conditioning but uses a fixed LCB coefficient $\beta$. BAPR extends this direction by adding BOCD belief tracking and adaptive $\beta$ modulation, while the contraction analysis applies to the frozen abstract operator.

\subsection{Ablation hierarchy}
\begin{center}
\begin{tabular}{lcccc}
\toprule
\textbf{Algorithm} & \textbf{Context} & \textbf{BOCD} & \textbf{Adaptive $\beta$} & \textbf{Formal property} \\
\midrule
...
$c_{\text{penalty}}$ & 0.5 & Penalty scale ($\beta_{\text{eff}} = \beta_{\text{base}} - \lambda_w c_{\text{penalty}}$); sweet spot per Sec.~\ref{sec:sensitivity} \\
