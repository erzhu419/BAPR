#!/usr/bin/env python3
"""Register and batch-submit the Ant V26 joint-controller DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v26-ant-joint-mode-risk"
SUBMIT_INTENT = "bapr-v26-ant-joint-mode-risk-submit"
...
def _gpu_command(variant: str, values: list[str]) -> str:
    return (
        f"SCHEDULEURM_ETA_TOTAL_UNITS={protocol.FINAL_NEXT_ITERATION[variant]} "
        ...
        "run_regime_polarity_ant_joint_mode_risk_v26 "
...
def common_stage_inputs() -> list[str]:
    return [
        ...
        str(ROOT / "scripts"),
        str(protocol.parent.REGISTRATION_ROOT),
        str(protocol.parent.ANALYSIS_ROOT),
        str(protocol.parent.parent.REGISTRATION_ROOT),
        str(protocol.parent.parent.ANALYSIS_ROOT),
        str(protocol.parent.parent.parent.SOURCE_BUNDLE_ROOT),
    ]


def train_signature(variant: str, seed: int) -> str:
    return f"{SIGNATURE_PREFIX}/train/{variant}/seed-{seed}"


def train_spec(variant: str, seed: int, priority: str) -> dict:
    run = protocol.run_dir(variant, seed)
    bundle = protocol.bundle_dir(variant, seed)
    return {
        "description": f"Ant V26 {variant} joint-mode risk seed {seed}",
        "cmd": _gpu_command(
            variant,
            ["--variant", variant, "--seed", str(seed), "--resume"],
        ),
        "cwd": str(ROOT),
        "signature": train_signature(variant, seed),
        "project": "BAPR",
        "vram_resource_family": "BAPR/ant-v26/joint-mode-risk/gpu-runtime",
        "vram": 3300,
        "ram_mb": 7168,
        "cpu": 2,
        "priority": priority,
...
def audit_spec(variant: str, seed: int, priority: str) -> dict:
    return {
        "description": f"Ant V26 {variant} joint-controller audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_ant_joint_mode_risk_audit_v26",
            ["--variant", variant, "--seed", str(seed), "--resume"],
        ),
        "cwd": str(ROOT),
        ...
        "stage_input_paths": [
            *common_stage_inputs(),
            str(protocol.bundle_dir(variant, seed)),
        ],
        "stage_excludes": ["paper/", "__pycache__/"],
        "reroute_on_node_down": True,
        "allow_initial_resume_scan_error": True,
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate Ant V26 joint-controller decision",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_ant_joint_mode_risk_v26",
            [],
        ),
        "cwd": str(ROOT),
        ...
        "stage_input_paths": [*common_stage_inputs(), str(protocol.AUDIT_ROOT)],
        "stage_excludes": ["paper/", "__pycache__/"],
        "reroute_on_node_down": True,
        "allow_initial_resume_scan_error": True,
        "allow_cpu_training": True,
...
def candidates(priority: str):
    ...
    for variant in protocol.VARIANTS:
        for seed in protocol.TRAINING_SEEDS:
            rows.append((
                train_signature(variant, seed),
                train_spec(variant, seed, priority),
...
def _submit_jsonl(specs: list[dict]) -> list[str]:
    ...
    if len(task_ids) != len(specs) or any(not task_id for task_id in task_ids):
        raise RuntimeError(
            "scheduler did not account for every Ant V26 task: "
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No Ant V26 tasks to submit")
        return
