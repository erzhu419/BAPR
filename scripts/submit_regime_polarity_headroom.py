#!/usr/bin/env python3
"""Submit the actuator-polarity oracle-headroom screen via scheduleurm."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-headroom/v1"
...
def _gpu_command(values: list[str]) -> str:
    return (
        ...
        "run_regime_polarity_headroom_controller "
...
def training_spec(
    ...
    return {
        "description": (
            f"Polarity headroom {protocol.env_slug(env)} {role} seed {seed}"),
        "cmd": _gpu_command([
...
def audit_spec(env: str, role: str, seed: int, priority: str) -> dict:
    ...
    return {
        "description": (
            f"Polarity strict audit {protocol.env_slug(env)} "
            f"{role} seed {seed}"),
        "cmd": _cpu_command(
            "jax_experiments.analysis.run_regime_polarity_headroom_audit",
        ...
        "stage_excludes": [
            ...
            "jax_experiments/results_regime_polarity_headroom_audit_v1/",
            "paper/",
        ],
        "reroute_on_node_down": True,
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate polarity oracle-headroom decision",
        "cmd": _cpu_command(
            "jax_experiments.analysis.analyze_regime_polarity_headroom", []),
        "cwd": str(ROOT),
        ...
        "stage_excludes": [
            "jax_experiments/results_regime_polarity_headroom_v1/",
            "jax_experiments/eval_bundles_regime_polarity_headroom_v1/",
            "paper/",
        ],
        "reroute_on_node_down": True,
        "allow_initial_resume_scan_error": True,
...
def _submit_jsonl(specs: list[dict]) -> list[str]:
    ...
    command = [
        ...
        "--stdin", "--trusted", "--json",
        "--intent-label", "bapr-polarity-headroom-v1-submit",
        "--intent-ttl", "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label", "bapr-polarity-headroom-v1-dispatch",
        "--intent-ttl", "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No polarity headroom tasks to submit")
        return
