#!/usr/bin/env python3
"""Submit the fresh-seed CUSUM confirmation through scheduler only."""
...
SIGNATURE_PREFIX = "BAPR/v3-structured-channel/v5/cusum-confirmation"
...
def evaluation_spec(event_seed: int, priority: str):
    ...
    signature = f"{SIGNATURE_PREFIX}/event-seed-{event_seed}"
    spec = {
        "description": (
            "Fresh CUSUM return confirmation, structured channel, "
            f"event seed {event_seed}"),
        "cmd": command(
            "jax_experiments.analysis."
            "run_bapr_v3_structured_channel_confirmation",
...
def analysis_spec(priority: str):
    ...
    spec = {
        "description": "Aggregate frozen five-seed CUSUM confirmation",
        "cmd": command(
            "jax_experiments.analysis."
            "analyze_bapr_v3_structured_channel_confirmation",
            [], cpu_only=True),
        "cwd": str(ROOT),
        "signature": signature,
        "project": "BAPR",
...
def main() -> None:
    ...
    if args.phase == "evaluation":
        candidates = [evaluation_spec(seed, args.priority)
                      for seed in protocol.EVENT_SEEDS]
    else:
        missing = [seed for seed in protocol.EVENT_SEEDS
                   if not protocol.group_path(seed).is_file()]
        if missing:
            raise SystemExit(
                f"confirmation groups incomplete; missing={missing}")
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No confirmation tasks to submit")
        return
