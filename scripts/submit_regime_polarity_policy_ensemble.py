#!/usr/bin/env python3
"""Submit the checkpoint-only polarity policy-ensemble diagnostic."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-policy-ensemble/v1"
SUBMIT_INTENT_LABEL = "bapr-polarity-policy-ensemble-v1-submit"
...
def audit_signature(group: str, event_seed: int) -> str:
    return (
        f"{SIGNATURE_PREFIX}/audit/{protocol.require_group(group)}/"
...
def audit_spec(
    ...
    output = protocol.audit_dir(group, event_seed)
    return {
        "description": (
            f"Polarity policy ensemble {group} event {event_seed}"),
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_policy_ensemble_audit",
            ["--group", group, "--event-seed", str(event_seed), "--resume"],
        ),
        "cwd": str(ROOT),
        "signature": audit_signature(group, event_seed),
        "project": "BAPR",
        "vram": 0,
        "ram_mb": 12288,
        "cpu": 32,
        ...
        "wait_for_files": [
            str(protocol.final.MODEL_MANIFEST),
            str(protocol.final.MODEL_PATH),
            *(str(path) for path in protocol.source_required_paths(group)),
        ],
        "stage_input_paths": [
            str(protocol.final.MODEL_MANIFEST.parent),
...
def analysis_spec(priority: str) -> dict[str, object]:
    return {
        "description": "Aggregate polarity policy-ensemble diagnostic",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_policy_ensemble",
            [],
        ),
        "cwd": str(ROOT),
...
def candidates(phase: str, priority: str):
    ...
    if phase in ("all", "audit"):
        rows.extend([
            (
                audit_signature(group, event_seed),
                audit_spec(group, event_seed, priority),
                protocol.audit_manifest(group, event_seed),
            )
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-polarity-policy-ensemble-v1-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No polarity policy-ensemble tasks to submit")
        return
