#!/usr/bin/env python3
"""Submit checkpoint-only robust-inclusive utility audits."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-specialist-utility/v8-safe"
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    wait_for_files = [
        str(source.audit_manifest(seed)),
    ...
    stage_input_paths = [
        str(source.audit_dir(seed)),
        str(frozen_v5.MODEL_ROOT),
        *(str(path) for path in bundle_dirs),
    ]
    if seed in protocol.CAPACITY_AUDIT_SEEDS:
        wait_for_files += [
            str(capacity.audit_manifest(seed)),
            *(
                str(capacity.event_result(seed, event_seed))
                for event_seed in protocol.CALIBRATION_EVENT_SEEDS
            ),
        ]
        stage_input_paths.append(str(capacity.audit_dir(seed)))
    return {
        "description": f"Safe specialist utility holdout seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_specialist_safe_utility_audit_v8",
        ...
        "cpu_training_justification": (
            "Frozen-controller calibration and holdout evaluation; no updates."
        ),
    }
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate safe specialist utility holdouts",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_specialist_safe_utility_v8",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
        ...
        "stage_input_paths": [
            ...
            str(source.AUDIT_ROOT),
            str(capacity.AUDIT_ROOT),
            str(frozen_v5.MODEL_ROOT),
        ],
...
def _submit(specs: list[dict]) -> list[str]:
    ...
    command = [
        ...
        "--json",
        "--intent-label",
        "bapr-specialist-safe-utility-v8-submit",
        "--intent-ttl",
        "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-specialist-safe-utility-v8-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No safe specialist utility tasks to submit")
        return
