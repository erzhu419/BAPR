#!/usr/bin/env python3
"""Register and submit the v13 checkpoint-only estimator transfer audit."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v13-frozen-estimator-transfer"
SUBMIT_INTENT = "bapr-v13-frozen-estimator-transfer-submit"
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V13 frozen-estimator transfer audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_frozen_estimator_transfer_audit_v13",
        ...
        "wait_for_files": [
            ...
            *(str(path) for path in parent.source_required_paths(seed)),
            *(
                str(path)
                for mode in parent.MODES
                for path in parent.bundle_required_paths(
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate v13 frozen-estimator transfer audit",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_frozen_estimator_transfer_v13",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v13-frozen-estimator-transfer-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No v13 frozen-estimator tasks to submit")
        return
