#!/usr/bin/env python3
"""Submit the two-timescale sequence-router oracle ladder via scheduler."""
...
SIGNATURE = "BAPR/v3-structured-channel-sequence-router/v3/dual-oracle"
...
def train_spec(priority: str):
    command = (
        ...
        "jax_experiments.analysis.train_bapr_v3_sequence_router_dual "
        "&& echo DONE"
    )
    spec = {
        "description": (
            "Measure privileged fixed-window headroom for slow and "
            "switch-specialized causal routers"),
        "cmd": command,
        "cwd": str(ROOT),
        "signature": SIGNATURE,
        "project": "BAPR",
