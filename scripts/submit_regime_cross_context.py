#!/usr/bin/env python3
"""Submit event-grouped cross-context audits through scheduleurm."""
...
SIGNATURE_PREFIX = "BAPR/regime-cross-context/v1"
...
def _cpu_command(module: str, values: list[str]) -> str:
    return (
        ...
        "XLA_FLAGS='--xla_cpu_multi_thread_eigen=false intra_op_parallelism_threads=1' OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 MKL_NUM_THREADS=1 "
        "NUMEXPR_NUM_THREADS=1 JAX_NUM_THREADS=1 JAX_CPU_ENABLE_ASYNC_DISPATCH=false "
...
def audit_signature(env: str, seed: int, event_seed: int) -> str:
    return (
        f"{SIGNATURE_PREFIX}/audit/{base.env_slug(env)}/"
...
def audit_spec(
    ...
    return {
        "description": (
            f"Cross-context {base.env_slug(env)} seed {seed} "
            f"event {event_seed}"),
        "cmd": _cpu_command(
            "jax_experiments.analysis.run_regime_cross_context_audit",
...
def analysis_spec(priority: str) -> dict[str, object]:
    return {
        "description": "Aggregate event-grouped cross-context diagnosis",
        "cmd": _cpu_command(
            "jax_experiments.analysis.analyze_regime_cross_context", []),
        "cwd": str(ROOT),
...
def _submit_jsonl(specs: list[dict[str, object]]) -> list[str]:
    ...
    command = [
        ...
        "--stdin", "--trusted", "--json",
        "--intent-label", "bapr-regime-cross-context-v1-submit",
        "--intent-ttl", "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label", "bapr-regime-cross-context-v1-dispatch",
        "--intent-ttl", "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No cross-context tasks to submit")
        return
