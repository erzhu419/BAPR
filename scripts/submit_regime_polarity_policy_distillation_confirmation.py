#!/usr/bin/env python3
"""Submit the frozen five-event policy-distillation confirmation."""
...
SIGNATURE_PREFIX = (
    "BAPR/regime-polarity-policy-distillation-confirmation/v1")
SUBMIT_INTENT_LABEL = "bapr-polarity-distillation-confirmation-v1-submit"
...
def audit_spec(event_seed: int, priority: str):
    ...
    return {
        "description": (
            "Frozen polarity distillation confirmation "
            f"event {event_seed}"),
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_policy_distillation_confirmation_audit",
...
def analysis_spec(priority: str):
    return {
        "description": "Aggregate frozen policy-distillation confirmation",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_policy_distillation_confirmation",
            [],
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids):
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-polarity-distillation-confirmation-v1-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No policy-distillation confirmation tasks to submit")
        return
