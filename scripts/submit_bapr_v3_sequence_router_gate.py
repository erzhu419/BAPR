#!/usr/bin/env python3
"""Submit the causal slow/fast selector through scheduler only."""
...
SIGNATURE = "BAPR/v3-structured-channel-sequence-router/v4/causal-gate"
...
def train_spec(priority: str):
    command = (
        ...
        "jax_experiments.analysis.train_bapr_v3_sequence_router_gate "
        "&& echo DONE"
    )
    spec = {
        "description": (
            "Train a causal selector over frozen slow and fast sequence "
            "routers on independent internal streams"),
        "cmd": command,
        "cwd": str(ROOT),
        "signature": SIGNATURE,
        "project": "BAPR",
