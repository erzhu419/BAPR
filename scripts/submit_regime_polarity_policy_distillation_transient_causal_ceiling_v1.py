#!/usr/bin/env python3
"""Submit the frozen delayed-oracle causal-ceiling audit graph."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-transient-causal-ceiling/v1"
SUBMIT_INTENT_LABEL = "bapr-transient-causal-ceiling-v1-submit"
...
def _source_wait_files(student_seed: int) -> list[str]:
    ...
    return [
        str(parent.development.ensemble.final.MODEL_MANIFEST),
        str(parent.development.ensemble.final.MODEL_PATH),
        *(str(path) for path in parent.development.source_required_paths(
            protocol.TEACHER_GROUP)),
        str(parent.development.model_manifest(
            protocol.TEACHER_GROUP, student_seed)),
        str(parent.development.model_path(
            protocol.TEACHER_GROUP, student_seed)),
        str(parent.parent.selection_manifest()),
        str(parent.parent.analysis_json()),
        str(parent.analysis_json()),
    ]
...
def _source_stage_inputs(student_seed: int) -> list[str]:
    parent = protocol.parent
    return [
        str(parent.development.ensemble.final.MODEL_MANIFEST.parent),
        *(str(path) for path in parent.development.source_bundle_dirs(
            protocol.TEACHER_GROUP)),
        str(parent.development.model_dir(
            protocol.TEACHER_GROUP, student_seed)),
        str(parent.parent.SELECTION_ROOT),
        str(parent.parent.ANALYSIS_ROOT),
...
def audit_spec(student_seed: int, event_seed: int, priority: str) -> dict:
    ...
    spec = _common_spec(
        f"Delayed-oracle ceiling student {student_seed} event {event_seed}",
        _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_policy_distillation_transient_causal_ceiling_audit_v1",
...
def analysis_spec(priority: str) -> dict:
    spec = _common_spec(
        "Aggregate transient delayed-oracle causal ceiling",
        _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_policy_distillation_transient_causal_ceiling_v1",
            [],
        ),
        analysis_signature(),
...
def _dispatch(task_ids):
    ...
    command = [
        ...
        "--intent-label", "bapr-transient-causal-ceiling-v1-dispatch",
        "--intent-ttl", "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No transient causal-ceiling tasks to submit")
        return
