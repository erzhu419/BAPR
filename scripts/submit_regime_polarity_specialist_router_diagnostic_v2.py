#!/usr/bin/env python3
"""Submit the switching-only specialist-router failure decomposition."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-specialist-router-diagnostic/v2"
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"Specialist-router causal diagnosis seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis.run_regime_polarity_specialist_router_diagnostic_v2",
        ...
        "cpu_training_justification": (
            "Fixed-checkpoint switching diagnosis only; no parameter updates."
        ),
    }
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate specialist-router causal diagnosis",
        "cmd": _cpu_command(
            "jax_experiments.analysis.analyze_regime_polarity_specialist_router_diagnostic_v2",
            [], threads=2),
        "cwd": str(ROOT),
...
def _submit(specs: list[dict]) -> list[str]:
    ...
    command = [
        ...
        "--intent-label", "bapr-specialist-router-diagnostic-v2-submit",
        "--intent-ttl", "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    command = [
        ...
        "bapr-specialist-router-diagnostic-v2-dispatch",
        "--intent-ttl", "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No specialist diagnostic tasks to submit")
        return
