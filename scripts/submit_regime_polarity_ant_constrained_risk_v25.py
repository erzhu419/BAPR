#!/usr/bin/env python3
"""Register and batch-submit the Ant V25 constrained-risk DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v25-ant-constrained-risk"
SUBMIT_INTENT = "bapr-v25-ant-constrained-risk-submit"
...
def common_stage_inputs() -> list[str]:
    return [
        ...
        str(protocol.parent.parent.SOURCE_BUNDLE_ROOT),
        str(protocol.parent.PRIOR_ANALYSIS_ROOT),
    ]


def specialist_signature(variant: str, seed: int, mode: int) -> str:
    return f"{SIGNATURE_PREFIX}/train/{variant}/seed-{seed}/mode-{mode}"
...
def specialist_spec(
    ...
    return {
        "description": (
            f"Ant V25 {variant} constrained-risk seed {seed} mode {mode}"),
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_ant_constrained_risk_v25",
        ...
        "project": "BAPR",
        "vram_resource_family": "BAPR/ant-v25/risk-sac/gpu-runtime",
        "vram": 3300,
        "ram_mb": 7168,
        "cpu": 2,
        "priority": priority,
...
def audit_spec(variant: str, seed: int, priority: str) -> dict:
    ...
    for mode in protocol.MODES:
        required.extend(str(path) for path in protocol.bundle_required_paths(
            variant, seed, mode))
    return {
        "description": f"Ant V25 {variant} policy-bank audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_ant_constrained_risk_audit_v25",
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate Ant V25 constrained-risk decision",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_ant_constrained_risk_v25",
            [],
        ),
        "cwd": str(ROOT),
...
def _submit_jsonl(specs: list[dict]) -> list[str]:
    ...
    if len(task_ids) != len(specs) or any(not task_id for task_id in task_ids):
        raise RuntimeError(
            "scheduler did not account for every Ant V25 task: "
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No Ant V25 tasks to submit")
        return
