#!/usr/bin/env python3
"""Submit the posterior-conditioned residual screen through scheduler only."""
...
SIGNATURE_PREFIX = "BAPR/v3-structured-channel/v6/posterior-residual-screen"
...
def evaluation_spec(variant: str, event_seed: int, priority: str):
    output = protocol.group_path(variant, event_seed)
    signature = f"{SIGNATURE_PREFIX}/{variant}/event-seed-{event_seed}"
    spec = {
        "description": (
            "Posterior-conditioned residual dev screen, "
            f"{variant}, event seed {event_seed}"),
        "cmd": command(
            "jax_experiments.analysis."
            "run_bapr_v3_posterior_residual_screen",
            ["--variant", variant, "--event-seed", str(event_seed),
             "--resume"],
            cpu_only=False),
        "cwd": str(ROOT),
        "signature": signature,
        ...
        "cpu": 2,
        "priority": priority,
        # Controller bundles and the frozen router currently live on this node.
...
def analysis_spec(priority: str):
    ...
    spec = {
        "description": "Aggregate posterior-conditioned residual dev screen",
        "cmd": command(
            "jax_experiments.analysis."
            "analyze_bapr_v3_posterior_residual_screen",
            [], cpu_only=True),
        "cwd": str(ROOT),
        "signature": signature,
        "project": "BAPR",
...
def main() -> None:
    ...
    parser.add_argument("--phase", choices=("evaluation", "analysis"),
                        required=True)
    ...
    if args.phase == "evaluation":
        candidates = [
            evaluation_spec(variant, seed, args.priority)
            for variant in protocol.RESIDUAL_VARIANTS
        ...
        missing = [
            ...
            for variant in protocol.RESIDUAL_VARIANTS
            for seed in protocol.DEVELOPMENT_EVENT_SEEDS
            if not protocol.group_path(variant, seed).is_file()
        ]
        if missing:
            raise SystemExit(f"residual screen groups incomplete: {missing}")
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No posterior-residual tasks to submit")
        return
