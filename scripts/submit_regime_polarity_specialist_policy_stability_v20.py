#!/usr/bin/env python3
"""Register and submit the v20 specialist policy-stability DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v20-specialist-policy-stability"
SUBMIT_INTENT = "bapr-v20-specialist-policy-stability-submit"
...
def source_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V20 fresh robust source seed {seed}",
        "cmd": _gpu_command(
            "jax_experiments.analysis.run_regime_polarity_robust_source_v20",
...
def specialist_spec(
    ...
    return {
        "description": f"V20 {variant} specialist seed {seed} mode {mode}",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_specialist_policy_stability_v20",
            [
                "--variant", variant,
                "--seed", str(seed),
                "--mode", str(mode),
...
def audit_spec(variant: str, seed: int, priority: str) -> dict:
    ...
    seed = protocol.require_training_seed(seed)
    return {
        "description": f"V20 {variant} audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_specialist_policy_stability_audit_v20",
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate v20 specialist policy-stability screen",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_specialist_policy_stability_v20",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v20-specialist-policy-stability-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No v20 specialist policy-stability tasks to submit")
        return
