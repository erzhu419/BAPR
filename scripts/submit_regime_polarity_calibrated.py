#!/usr/bin/env python3
"""Submit the v2 calibrated-posterior screen through scheduleurm."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-evidence-calibration/v2"
...
def training_spec(priority: str) -> dict[str, object]:
    ...
    return {
        "description": "Fit causal polarity evidence calibration",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "train_regime_polarity_evidence_calibration",
            [],
        ),
        "cwd": str(ROOT),
        ...
        "vram_resource_family": (
            "BAPR/regime-polarity/evidence-calibration/gpu-runtime"),
        ...
        "stage_input_paths": [
            str(v1.MODEL_ROOT),
            *[str(path) for path in source_bundles],
        ],
        "stage_excludes": [
...
def audit_spec(seed: int, priority: str) -> dict[str, object]:
    ...
    return {
        "description": f"Audit calibrated polarity posterior seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_calibrated_audit",
        ...
        "cpu_training_justification": (
            "Frozen deterministic controller and calibrated-estimator "
...
def analysis_spec(priority: str) -> dict[str, object]:
    return {
        "description": "Aggregate calibrated polarity posterior screen",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_calibrated",
            [],
        ),
        "cwd": str(ROOT),
...
def _submit_jsonl(specs: list[dict[str, object]]) -> list[str]:
    ...
    command = [
        ...
        "--json",
        "--intent-label",
        "bapr-polarity-evidence-v2-submit",
        "--intent-ttl",
        "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-polarity-evidence-v2-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No calibrated posterior tasks to submit")
        return
