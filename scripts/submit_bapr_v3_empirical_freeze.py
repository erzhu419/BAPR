#!/usr/bin/env python3
"""Submit the BAPR-v3 empirical-variance/frozen-teacher seed0 screen."""
...
SAVE_ROOT = ROOT / "jax_experiments" / "results_bapr_v3_empirical_freeze"
...
VARIANTS = (
    Variant("nll_freeze", "mode_calibrated", 0.05),
    Variant("emp05_freeze", "mode_empirical", 0.05),
)
...
def training_values(variant: Variant, family: str, env: str, seed: int,
    ...
    return [
        ...
        "--bapr_v2_gate_supervision_weight", "0.0",
        "--bapr_v2_unsafe_deviation_weight", "0.0",
        ...
        "--bapr_v3_variance_model", variant.variance_model,
        ...
        "--bapr_v3_variance_ema", str(variant.variance_ema),
...
def task_spec(variant: Variant, family: str, env: str, seed: int,
    ...
    return {
        "description": f"BAPR-v3 variance/freeze screen {run_name}",
        "cmd": command,
        "cwd": str(ROOT),
        "signature": (
            f"BAPR/v3-empirical-freeze/{variant.name}/{family}/"
            f"{env}/seed{seed}"),
        "project": "BAPR",
        "vram": 2600,
        "ram_mb": 4096,
...
def submit_jsonl(specs: list[dict]) -> list[str]:
    ...
    command = [
        ...
        "--stdin", "--trusted", "--json",
        "--intent-label", "bapr-v3-empirical-freeze-submit",
        "--intent-ttl", "900",
    ]
...
def dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--intent-label", "bapr-v3-empirical-freeze-dispatch",
        "--intent-ttl", "900",
    ]
...
def main() -> None:
    ...
    if expected > 12:
        raise SystemExit(
            f"variance/freeze screen expands to {expected}; cap is 12 tasks")
    ...
    print(
        f"BAPR-v3 empirical/freeze matrix: expected={expected} "
