#!/usr/bin/env python3
"""Register and submit the v14 checkpoint-only fallback-router screen."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v14-conflict-fallback-router"
SUBMIT_INTENT = "bapr-v14-conflict-fallback-router-submit"
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V14 conflict-fallback router audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_conflict_fallback_router_audit_v14",
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate v14 conflict-fallback router screen",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_conflict_fallback_router_v14",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v14-conflict-fallback-router-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No v14 conflict-fallback tasks to submit")
        return
