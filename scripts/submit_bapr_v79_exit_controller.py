#!/usr/bin/env python3
"""Submit the v79 BAPR exit-controller screening batch.

Round 1 stays small: 3 controller-exit variants on Ant seeds 0-4 plus
single-seed canaries on HalfCheetah/Hopper/Walker2d.
"""
...
def exit_flags(
    *,
    ...
    min_active: int,
    cooldown: int,
    signal_exit: float = 0.0,
    improve_exit: float = 0.0,
    drop: float = 0.45,
    ramp: float = 0.55,
    ...
    flags = [
        ...
        "--bapr_controller_recent_multiplier", "1.0",
        "--bapr_controller_recent_add_frac", "0.0",
        "--bapr_controller_lcb_multiplier", "1.0",
        "--bapr_controller_actor_update_multiplier", str(actor),
        "--bapr_controller_max_active_iters", str(max_active),
        "--bapr_controller_min_active_iters", str(min_active),
        "--bapr_controller_exit_cooldown_iters", str(cooldown),
    ]
    if signal_exit > 0:
        flags += ["--bapr_controller_exit_signal_threshold", str(signal_exit)]
    if improve_exit > 0:
        flags += ["--bapr_controller_exit_improve_frac", str(improve_exit)]
    if actor_recover:
...
VARIANTS = [
    Variant(
        "v79a_exit80",
        exit_flags(reg=0.70, actor=0.85, max_active=80, min_active=40,
                   cooldown=80, signal_exit=0.08),
    ),
    Variant(
        "v79b_exit120_regsafe",
        exit_flags(reg=0.85, actor=0.90, max_active=120, min_active=50,
                   cooldown=80, signal_exit=0.06),
    ),
    Variant(
        "v79c_improve_exit",
        exit_flags(reg=0.75, actor=0.90, max_active=160, min_active=40,
                   cooldown=80, signal_exit=0.06, improve_exit=0.008,
                   actor_recover=80, reg_recover=80),
    ),
]
...
def submit_job(variant: Variant, env: str, seed: int, run_name: str,
    ...
    sched_cmd = [
        ...
        "--description", f"BAPR v79 exit controller: {run_name}",
        "--cmd", cmd,
        "--cwd", str(ROOT),
        "--signature", f"BAPR/v79-exit/{variant.name}/{env}/seed{seed}",
