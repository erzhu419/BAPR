#!/usr/bin/env python3
"""Register and batch-submit the Ant V24 switch-recovery DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v24-ant-switch-recovery"
SUBMIT_INTENT = "bapr-v24-ant-switch-recovery-submit"
GPU_NODES = ["jtl110gpu", "jtl311linux", "node007"]
CPU_NODES = [
...
def common_stage_inputs() -> list[str]:
    return [
        ...
        str(protocol.parent.SOURCE_BUNDLE_ROOT),
        str(protocol.PRIOR_ANALYSIS_ROOT),
    ]
...
def specialist_spec(
    ...
    return {
        "description": (
            f"Ant V24 {variant} switch-recovery seed {seed} mode {mode}"),
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_ant_switch_recovery_v24",
...
def audit_spec(variant: str, seed: int, priority: str) -> dict:
    ...
    for mode in protocol.MODES:
        required.extend(str(path) for path in protocol.bundle_required_paths(
            variant, seed, mode))
    return {
        "description": f"Ant V24 {variant} policy-bank audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_ant_switch_recovery_audit_v24",
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate Ant V24 switch-recovery decision",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_ant_switch_recovery_v24",
            [],
        ),
        "cwd": str(ROOT),
...
def _submit_jsonl(specs: list[dict]) -> list[str]:
    ...
    if len(task_ids) != len(specs) or any(not task_id for task_id in task_ids):
        raise RuntimeError(
            "scheduler did not account for every Ant V24 task: "
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No Ant V24 tasks to submit")
        return
