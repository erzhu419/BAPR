#!/usr/bin/env python3
"""Register and submit the gated fresh ten-seed confirmation DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-deployment-confirmation/v1"
SUBMIT_INTENT = "bapr-polarity-deployment-confirmation-v1-submit"
GPU_NODES = [
...
def _gate_waits() -> list[str]:
    return [
        str(protocol.REGISTRATION_PATH),
        str(mechanism.analysis_json()),
        str(mechanism.pass_marker()),
    ]
...
def _artifact_stage_inputs() -> list[str]:
    inputs = [
        ...
        str(mechanism.ANALYSIS_ROOT),
    ]
    for path in protocol.registration_source_paths():
        relative = path.relative_to(ROOT).as_posix()
        if (relative.startswith("jax_experiments/results_")
                or relative.startswith("jax_experiments/eval_bundles_")):
            inputs.append(str(path.parent))
    return list(dict.fromkeys(inputs))


def _student_stage_inputs() -> list[str]:
    return list(dict.fromkeys([
        *_artifact_stage_inputs(),
...
def student_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"Fresh confirmation BAPR student seed {seed}",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "train_regime_polarity_deployment_confirmation_student_v1",
            ["--variant", "mode_heads", "--student-seed", str(seed),
             "--resume"],
            1 + protocol.DAGGER_ROUNDS),
        "cwd": str(ROOT),
        "signature": train_signature("bapr", seed),
        "project": "BAPR",
        "vram_resource_family": (
            "BAPR/polarity-confirmation/frozen-student-runtime"),
        "vram": 2400,
        ...
        "wait_for_files": [
            ...
            *(str(path) for path in
              protocol.source_required_paths("mode_heads")),
        ],
...
def sac_spec(seed: int, priority: str) -> dict:
    seed = protocol.require_seed(seed)
    run = protocol.baseline_run_dir("sac", seed)
    bundle = protocol.baseline_bundle_dir("sac", seed)
    return {
        "description": f"Fresh confirmation SAC seed {seed}",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_deployment_confirmation_sac_v1",
            ["--seed", str(seed), "--resume"], protocol.MAX_ITERS),
        "cwd": str(ROOT),
        "signature": train_signature("sac", seed),
        "project": "BAPR",
        "vram_resource_family": "BAPR/polarity-confirmation/sac-runtime",
        "vram": 2300,
        "allow_gpu_over_one_third": True,
        "ram_mb": 8192,
        "cpu": 2,
...
def corrected_spec(method: str, seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"Fresh confirmation {method} seed {seed}",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_deployment_confirmation_corrected_v1",
        ...
        "project": "BAPR",
        "vram_resource_family": (
            "BAPR/polarity-confirmation/recurrent-escp"
            if recurrent else "BAPR/polarity-confirmation/resac-b0"),
...
def audit_spec(method: str, seed: int, priority: str) -> dict:
    ...
    if method == "bapr":
        ...
        waits = [
            *_gate_waits(),
            *(str(path) for path in
              protocol.baseline_bundle_required_paths("sac", seed)),
        ]
        stages = [
            *_artifact_stage_inputs(),
            str(protocol.baseline_bundle_dir("sac", seed)),
        ]
    else:
        waits = [
            *_gate_waits(),
            *(str(path) for path in protocol.bundle_required_paths(method, seed)),
        ]
        stages = [
            *_artifact_stage_inputs(), str(protocol.bundle_dir(method, seed)),
        ]
    output = protocol.audit_dir(method, seed)
    return {
        "description": f"Fresh confirmation strict audit {method} seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_deployment_confirmation_audit_v1",
            ["--method", method, "--seed", str(seed), "--resume"]),
        "cwd": str(ROOT),
        ...
        "stage_input_paths": list(dict.fromkeys(stages)),
        "stage_excludes": [
            "jax_experiments/results*/",
            "jax_experiments/eval_bundles*/",
            "paper/",
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate fresh ten-seed BAPR deployment confirmation",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_deployment_confirmation_v1", []),
        "cwd": str(ROOT),
...
def candidates(phase: str, priority: str):
    ...
    if phase in ("all", "train"):
        rows.extend([
            (train_signature("bapr", seed), student_spec(seed, priority),
             protocol.model_manifest("mode_heads", seed))
            for seed in protocol.TRAINING_SEEDS
        ])
        rows.extend([
            (train_signature("sac", seed), sac_spec(seed, priority),
             protocol.baseline_bundle_manifest("sac", seed))
            for seed in protocol.TRAINING_SEEDS
        ])
        rows.extend([
            (train_signature(method, seed),
             corrected_spec(method, seed, priority),
    ...
    if phase in ("all", "audit"):
        rows.extend([
            (audit_signature(method, seed), audit_spec(method, seed, priority),
...
def _submit_jsonl(specs: list[dict]) -> list[str]:
    ...
    task_ids = [str(item.get("id", ""))
                for item in response.get("submitted", [])]
    if (len(task_ids) != len(specs) or any(not value for value in task_ids)
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "bapr-polarity-deployment-confirmation-v1-dispatch",
        "--intent-ttl", "900",
    ]
...
def main() -> None:
    ...
    parser.add_argument("--phase", choices=("all", "train", "audit", "analysis"),
                        default="all")
    ...
    if args.register or args.register_only:
        protocol.create_registration()
    protocol.validate_registration()
    if args.register_only:
        print(f"REGISTERED ONLY: {protocol.REGISTRATION_PATH}")
        return
    release = protocol.validate_mechanism_release()
    print("MECHANISM RELEASE VERIFIED: " + json.dumps(release, sort_keys=True))
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No deployment-confirmation tasks to submit")
        return
