#!/usr/bin/env python3
"""Submit the CPU-only causal specialist-router development screen."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-specialist-router/v1"
SUBMIT_INTENT = "bapr-regime-polarity-specialist-router-v1-submit"
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"Causal specialist-router audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis.run_regime_polarity_specialist_router_audit_v1",
        ...
        "cpu_training_justification": (
            "Fixed-checkpoint causal evaluation only; no parameter updates."
        ),
    }
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate causal specialist-router screen",
        "cmd": _cpu_command(
            "jax_experiments.analysis.analyze_regime_polarity_specialist_router_v1",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-regime-polarity-specialist-router-v1-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No specialist-router tasks to submit")
        return
