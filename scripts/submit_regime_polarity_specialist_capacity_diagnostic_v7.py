#!/usr/bin/env python3
"""Submit the checkpoint-only v7 specialist-capacity diagnostic."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-specialist-capacity/v7-stationary"
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"Stationary specialist-capacity audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_specialist_capacity_diagnostic_v7",
        ...
        "cpu_training_justification": (
            "Frozen-controller stationary evaluation; no updates."
        ),
    }
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate stationary specialist-capacity diagnostic",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_specialist_capacity_diagnostic_v7",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def _submit(specs: list[dict]) -> list[str]:
    ...
    command = [
        ...
        "--json",
        "--intent-label",
        "bapr-specialist-capacity-v7-submit",
        "--intent-ttl",
        "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-specialist-capacity-v7-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No stationary specialist-capacity tasks to submit")
        return
