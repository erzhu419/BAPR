#!/usr/bin/env python3
"""Submit privileged fallback-oracle audits through scheduler only."""
...
SIGNATURE_PREFIX = "BAPR/v3-structured-channel/v9/fallback-oracle"
...
def evaluation_spec(event_seed: int, priority: str):
    ...
    spec = {
        "description": (
            "Privileged true-mode fallback-oracle audit, "
            f"event seed {event_seed}"),
        "cmd": command(
            "jax_experiments.analysis.run_bapr_v3_fallback_oracle",
...
def analysis_spec(priority: str):
    ...
    spec = {
        "description": "Aggregate privileged fallback-oracle capacity audit",
        "cmd": command(
            "jax_experiments.analysis.analyze_bapr_v3_fallback_oracle",
            [], cpu_only=True),
        "cwd": str(ROOT),
        "signature": signature,
        "project": "BAPR",
...
def main() -> None:
    ...
    candidates = (
        ...
        else [analysis_spec(args.priority)]
    )
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No fallback-oracle tasks to submit")
        return
