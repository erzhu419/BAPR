#!/usr/bin/env python3
"""Submit the sequence-router internal diagnostic through scheduler only."""
...
SIGNATURE = "BAPR/v3-structured-channel-sequence-router/v1/diagnostic"


def diagnostic_spec(priority: str):
    command = (
        ...
        "jax_experiments.analysis.diagnose_bapr_v3_sequence_router "
        "&& echo DONE"
    )
    spec = {
        "description": (
            "Diagnose sequence-router modes, switch lag, and fallback on "
        ...
        "result_dir": str(diagnostic.OUTPUT_ROOT),
        "local_result_dir": str(diagnostic.OUTPUT_ROOT),
        "stage_excludes": [
    ...
    return SIGNATURE, spec, diagnostic.JSON_PATH
...
def main() -> None:
    ...
    signature, spec, output = diagnostic_spec(args.priority)
