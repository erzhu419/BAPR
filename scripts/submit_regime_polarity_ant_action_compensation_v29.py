#!/usr/bin/env python3
"""Register and submit the no-training V29 Ant compensation DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v29-ant-action-compensation"
SUBMIT_INTENT = "bapr-v29-ant-action-compensation-submit"
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V29 Ant oracle action compensation audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_ant_action_compensation_audit_v29",
        ...
        "stage_input_paths": [
            ...
            *(str(protocol.specialist_bundle(mode, seed))
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate V29 Ant oracle compensation decision",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_ant_action_compensation_v29",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v29-ant-action-compensation-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print(f"V29 dry-run task count: {len(specs)}", flush=True)
        return
    if not specs:
        print("No V29 Ant action-compensation tasks to submit")
        return
