#!/usr/bin/env python3
"""Submit the frozen mode-head policy-compression confirmation."""
...
SIGNATURE_PREFIX = (
    "BAPR/regime-polarity-policy-distillation-control-confirmation/v2"
)
SUBMIT_INTENT_LABEL = "bapr-polarity-control-confirmation-v2-submit"
...
def audit_spec(event_seed: int, priority: str):
    ...
    return {
        "description": (
            "Frozen mode-head compression confirmation "
            f"event {event_seed}"
        ),
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_policy_distillation_control_confirmation_audit_v2",
        ...
        "cpu_training_justification": (
            "Strict checkpoint-only deterministic confirmation audit; "
            "no policy or estimator updates."
        ),
    }
...
def analysis_spec(priority: str):
    return {
        "description": "Aggregate frozen mode-head compression confirmation",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_policy_distillation_control_confirmation_v2",
            [],
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids):
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-polarity-control-confirmation-v2-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No mode-head confirmation tasks to submit")
        return
