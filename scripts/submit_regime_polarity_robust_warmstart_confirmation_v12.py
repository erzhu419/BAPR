#!/usr/bin/env python3
"""Register and submit the frozen v12 five-seed confirmation DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v12-warmstart-confirmation"
SUBMIT_INTENT = "bapr-v12-warmstart-confirmation-submit"
...
def registration_data_files() -> list[str]:
    """Non-code inputs re-read by ``validate_registration`` on workers."""
    return [
        str(path)
...
def source_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V12 fresh robust source seed {seed}",
        "cmd": _gpu_command(
            "jax_experiments.analysis.run_regime_polarity_robust_source_v12",
...
def specialist_spec(seed: int, mode: int, priority: str) -> dict:
    ...
    return {
        "description": f"V12 actor-only specialist seed {seed} mode {mode}",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_robust_warmstart_specialist_v12",
...
def audit_spec(seed: int, priority: str) -> dict:
    seed = protocol.require_training_seed(seed)
    return {
        "description": f"V12 policy-bank confirmation audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate v12 actor-only five-seed confirmation",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_robust_warmstart_confirmation_v12",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v12-warmstart-confirmation-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No v12 warm-start confirmation tasks to submit")
        return
