#!/usr/bin/env python3
"""Register and batch-submit the V21 full-state confirmation DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v21-full-state-confirmation"
SUBMIT_INTENT = "bapr-v21-full-state-confirmation-submit"
...
def _gpu_command(module: str, values: list[str], units: int,
                 memory_fraction: float) -> str:
    return (
...
def source_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V21 robust source seed {seed}",
        "cmd": _gpu_command(
            "jax_experiments.analysis.run_regime_polarity_robust_source_v21",
...
def specialist_signature(seed: int, mode: int) -> str:
    return (
        f"{SIGNATURE_PREFIX}/train/full-state-final/seed-"
        f"{protocol.require_training_seed(seed)}/mode-"
        f"{protocol.require_mode(mode)}"
    )


def specialist_spec(seed: int, mode: int, priority: str) -> dict:
    seed = protocol.require_training_seed(seed)
    mode = protocol.require_mode(mode)
    run = protocol.specialist_run_dir(seed, mode)
    bundle = protocol.specialist_bundle_dir(seed, mode)
    return {
        "description": f"V21 full-state specialist seed {seed} mode {mode}",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_full_state_specialist_v21",
            ["--seed", str(seed), "--mode", str(mode), "--resume"],
            protocol.SPECIALIST_FINAL_NEXT_ITERATION,
            0.20,
        ),
        "cwd": str(ROOT),
...
def baseline_spec(
    ...
    if kind == "sac_replica":
        if slot is None:
            raise ValueError("SAC5 producer requires a slot")
        ...
        description = f"V21 SAC5 replica seed {seed} slot {slot}"
    else:
        kind = protocol.require_method(kind)
        if slot is not None:
            raise ValueError("standard baseline cannot have a replica slot")
        ...
        fraction = 0.30
        description = f"V21 {kind} baseline seed {seed}"
    ...
    return {
        ...
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_full_state_confirmation_baseline_v21",
            values,
            protocol.MAX_ITERS,
            fraction,
        ),
        "cwd": str(ROOT),
        "signature": baseline_signature(kind, seed, slot),
        "project": "BAPR",
        "vram": vram,
        "ram_mb": ram_mb,
        "cpu": 2,
...
def _producer_required_paths(seed: int) -> list[str]:
    paths = [str(path) for path in protocol.source_required_paths(seed)]
    for mode in protocol.MODES:
        paths.extend(str(path) for path in protocol.specialist_required_paths(
            seed, mode))
...
def audit_spec(seed: int, priority: str) -> dict:
    seed = protocol.require_training_seed(seed)
    return {
        "description": f"V21 full-state confirmation audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate V21 full-state confirmation",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_full_state_confirmation_v21",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
...
def candidates(priority: str):
    ...
    for seed in protocol.TRAINING_SEEDS:
        ...
        for slot in protocol.SAC_REPLICA_SLOTS:
            rows.append((
                baseline_signature("sac_replica", seed, slot),
                baseline_spec("sac_replica", seed, slot, priority),
                protocol.bundle_manifest("sac_replica", seed, slot),
            ))
        for method in protocol.TRAINED_METHODS:
            rows.append((
                baseline_signature(method, seed),
                baseline_spec(method, seed, None, priority),
        ...
        for mode in protocol.MODES:
            rows.append((
                ...
                protocol.specialist_bundle_manifest(seed, mode),
            ))
        rows.append((
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v21-full-state-confirmation-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print(f"V21 dry-run task count: {len(specs)}", flush=True)
        return
    if not specs:
        print("No V21 full-state confirmation tasks to submit")
        return
