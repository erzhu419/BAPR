#!/usr/bin/env python3
"""Register and submit the no-training V30 Ant branch-risk DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v30-ant-branch-risk"
SUBMIT_INTENT = "bapr-v30-ant-branch-risk-submit"
...
def registration_data_input_dirs() -> list[str]:
    ...
    return list(dict.fromkeys(
        ...
        + [str(path.parent) for path in data_files]
    ))
...
def audit_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V30 Ant paired branch-risk audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_ant_branch_risk_audit_v30",
        ...
        "stage_input_paths": [
            *registration_data_input_dirs(),
            str(protocol.parent.audit_dir(seed)),
            str(protocol.source_bundle(seed)),
            str(protocol.specialist_bundle(reference_mode, seed)),
        ],
        ...
        "cpu_training_justification": (
            "Frozen-policy paired simulator-state evaluation; no updates."
        ),
    }
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate V30 Ant branch-risk headroom decision",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_ant_branch_risk_v30",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
        ...
        "allow_initial_resume_scan_error": True,
        "allow_cpu_training": True,
        "cpu_training_justification": "Immutable compact JSON aggregation.",
    }
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v30-ant-branch-risk-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        print(f"V30 dry-run task count: {len(specs)}", flush=True)
        return
    if not specs:
        print("No V30 Ant branch-risk tasks to submit")
        return
