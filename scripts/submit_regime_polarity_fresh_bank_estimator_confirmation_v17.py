#!/usr/bin/env python3
"""Register and submit the v17 fresh-policy-bank confirmation DAG."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity/v17-fresh-bank-estimator-confirmation"
SUBMIT_INTENT = "bapr-v17-fresh-bank-estimator-confirmation-submit"
...
def source_spec(seed: int, priority: str) -> dict:
    ...
    return {
        "description": f"V17 fresh robust source seed {seed}",
        "cmd": _gpu_command(
            "jax_experiments.analysis.run_regime_polarity_robust_source_v17",
...
def specialist_signature(seed: int, mode: int) -> str:
    return (
        f"{SIGNATURE_PREFIX}/train/actor-only/seed-"
...
def specialist_spec(seed: int, mode: int, priority: str) -> dict:
    ...
    run_dir = protocol.run_dir("actor_only", seed, mode)
    bundle = protocol.bundle_dir("actor_only", seed, mode)
    return {
        "description": f"V17 actor-only specialist seed {seed} mode {mode}",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_robust_warmstart_specialist_v17",
...
def audit_spec(seed: int, priority: str) -> dict:
    seed = protocol.require_training_seed(seed)
    return {
        "description": f"V17 estimator confirmation audit seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_fresh_bank_estimator_confirmation_audit_v17",
            ["--seed", str(seed), "--resume"],
            threads=16,
        ),
        "cwd": str(ROOT),
        ...
        "stage_input_paths": [
            *registration_data_input_dirs(),
            str(protocol.source_bundle(seed)),
            *(
                str(protocol.bundle_dir("actor_only", seed, mode))
                for mode in protocol.MODES
            ),
        ],
        "stage_excludes": ["paper/"],
        ...
        "cpu_training_justification": (
            "Frozen policy and estimator evaluation; no updates."
        ),
    }
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate v17 fresh-bank estimator confirmation",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_fresh_bank_estimator_confirmation_v17",
            [],
            threads=2,
        ),
        "cwd": str(ROOT),
    ...
    for seed in protocol.TRAINING_SEEDS:
        ...
        for mode in protocol.MODES:
            rows.append((
                ...
                protocol.bundle_manifest("actor_only", seed, mode),
            ))
        rows.append((
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-v17-fresh-bank-estimator-confirmation-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No v17 fresh-bank confirmation tasks to submit")
        return
