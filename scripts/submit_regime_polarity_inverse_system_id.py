#!/usr/bin/env python3
"""Submit the v3 inverse system-ID screen through scheduleurm."""
...
SIGNATURE_PREFIX = "BAPR/regime-polarity-inverse-system-id/v3"
...
def training_spec(priority: str) -> dict[str, object]:
    ...
    return {
        "description": "Fit polarity executed-action inverse system ID",
        "cmd": _gpu_command(
            "jax_experiments.analysis."
            "train_regime_polarity_inverse_system_id",
            ["--resume"],
        ),
        "cwd": str(ROOT),
        ...
        "vram_resource_family": (
            "BAPR/regime-polarity/inverse-system-id/gpu-runtime"),
...
def audit_spec(seed: int, priority: str) -> dict[str, object]:
    ...
    return {
        "description": f"Audit inverse system ID seed {seed}",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "run_regime_polarity_inverse_system_id_audit",
        ...
        "cpu_training_justification": (
            "Frozen deterministic controller and inverse-estimator "
...
def analysis_spec(priority: str) -> dict[str, object]:
    return {
        "description": "Aggregate polarity inverse system-ID screen",
        "cmd": _cpu_command(
            "jax_experiments.analysis."
            "analyze_regime_polarity_inverse_system_id",
            [],
        ),
        "cwd": str(ROOT),
...
def _submit_jsonl(specs: list[dict[str, object]]) -> list[str]:
    ...
    command = [
        ...
        "--json",
        "--intent-label",
        "bapr-polarity-inverse-system-id-v3-submit",
        "--intent-ttl",
        "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label",
        "bapr-polarity-inverse-system-id-v3-dispatch",
        "--intent-ttl",
        "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No inverse system-ID tasks to submit")
        return
