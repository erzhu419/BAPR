#!/usr/bin/env python3
"""Submit the complete BAPR-v6 optimizer-equivalent chain via scheduler."""
...
SIGNATURE_PREFIX = "BAPR/v6-balanced-option/v1"
RESOURCE_FAMILY = "BAPR/v6-balanced-option/gpu-runtime"
...
def training_spec(profile: str, priority: str):
    ...
    spec = v5_submit.common_gpu_spec(
        signature,
        f"BAPR-v6 optimizer-equivalent option {profile} training, seed "
        f"{protocol.TRAINING_SEED}",
        v5_submit.command(
            "jax_experiments.analysis.run_bapr_v6_balanced_option",
            ["--profile", profile, "--resume"],
            memory_fraction=0.76 if profile == "formal" else 0.40,
            eta_total_units=(
...
def audit_spec(event_seed: int, priority: str):
    ...
    spec = v5_submit.common_gpu_spec(
        signature,
        f"Strict BAPR-v6 balanced-option audit, event seed {event_seed}",
        v5_submit.command(
            "jax_experiments.analysis.run_bapr_v6_balanced_option_audit",
            ["--event-seed", str(event_seed), "--resume"],
            memory_fraction=0.45),
        output, protocol.checkpoint_dir("formal"),
        vram=v5_submit.COLD_START_VRAM_MB, ram_mb=24576,
        priority=priority, resource_family=RESOURCE_FAMILY,
    )
...
def analysis_spec(priority: str):
    ...
    spec = {
        "description": "Aggregate BAPR-v6 optimizer-equivalent screen",
        "cmd": (
            f"{v5_submit.LINUX_CPU_PYTHON} -u -m "
            "jax_experiments.analysis.analyze_bapr_v6_balanced_option "
            "&& echo DONE"
        ),
        "cwd": str(ROOT),
        "signature": signature,
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No BAPR-v6 tasks to submit")
        return
