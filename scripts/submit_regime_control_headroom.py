#!/usr/bin/env python3
"""Submit the frozen regime-control headroom chain through scheduleurm."""
...
SIGNATURE_PREFIX = "BAPR/regime-control-headroom/v1"
...
def _gpu_command(values: list[str]) -> str:
    return (
        ...
        "jax_experiments.analysis.run_regime_control_headroom_controller "
...
def training_spec(
    ...
    return {
        "description": (
            f"Regime headroom {protocol.env_slug(env)} {role} seed {seed}"),
        "cmd": _gpu_command([
        ...
        "vram_resource_family": (
            "BAPR/regime-control-headroom/regime-sac/gpu-runtime"),
...
def audit_spec(env: str, role: str, seed: int, priority: str) -> dict:
    ...
    return {
        "description": (
            f"Regime headroom strict audit {protocol.env_slug(env)} "
            f"{role} seed {seed}"),
        "cmd": _cpu_command(
            "jax_experiments.analysis.run_regime_control_headroom_audit",
            ["--env", env, "--role", role, "--seed", str(seed),
             "--resume"]),
        "cwd": str(ROOT),
        ...
        "stage_excludes": [
            "jax_experiments/results_regime_control_headroom_v1/",
            "jax_experiments/results_regime_control_headroom_audit_v1/",
            "paper/",
        ],
        "reroute_on_node_down": True,
...
def analysis_spec(priority: str) -> dict:
    return {
        "description": "Aggregate preregistered regime headroom decision",
        "cmd": _cpu_command(
            "jax_experiments.analysis.analyze_regime_control_headroom", []),
        "cwd": str(ROOT),
        ...
        "stage_excludes": [
            "jax_experiments/results_regime_control_headroom_v1/",
            "jax_experiments/eval_bundles_regime_control_headroom_v1/",
            "paper/",
        ],
        "reroute_on_node_down": True,
        "allow_initial_resume_scan_error": True,
...
def _submit_jsonl(specs: list[dict]) -> list[str]:
    ...
    command = [
        ...
        "--stdin", "--trusted", "--json",
        "--intent-label", "bapr-regime-headroom-v1-submit",
        "--intent-ttl", "900",
    ]
...
def _dispatch(task_ids: list[str]) -> None:
    ...
    command = [
        ...
        "--bulk-window",
        "--intent-label", "bapr-regime-headroom-v1-dispatch",
        "--intent-ttl", "900",
    ]
...
def main() -> None:
    ...
    if args.dry_run:
        ...
        return
    if not specs:
        print("No regime-control headroom tasks to submit")
        return
