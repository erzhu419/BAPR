"""Aggregate the posterior-discrete fallback-fill development audit."""
...
from jax_experiments.analysis import (
    analyze_bapr_v3_posterior_residual_policy as residual_analysis,
)
from jax_experiments.analysis import (
...
from jax_experiments.analysis import (
    run_bapr_v3_posterior_discrete_selector as runner,
)
from jax_experiments.analysis import (
...
METRICS = residual_analysis.METRICS
BASELINES = residual_analysis.BASELINES


def _routing_means(groups, metric: str):
    rows = []
    ...
    keys = (
        ...
        "action_delta_l2_mean", "wrong_route_rate", "median_switch_delay",
        "hard_fallback_rate", "fallback_replacement_rate",
    )
    return {
...
def summarize() -> dict[str, Any]:
    ...
    candidate = {
        metric: [
            residual_analysis.candidate_return(groups[seed], metric)
            for seed in protocol.DEVELOPMENT_EVENT_SEEDS
        ]
    ...
    baselines = {
        controller: {
            metric: [
                residual_analysis.baseline_return(
                    references[seed], controller, metric)
                for seed in protocol.DEVELOPMENT_EVENT_SEEDS
            ]
            for metric in METRICS
        }
    ...
    for metric in ("stationary", "switching_full_cycle"):
        termination_deltas[metric] = statistics.mean(
            residual_analysis.candidate_termination(groups[seed], metric)
            - screen_analysis.event_termination(
                references[seed], "learned_utility_router", metric)
            for seed in protocol.DEVELOPMENT_EVENT_SEEDS)
    routing = {metric: _routing_means(groups, metric) for metric in METRICS}
    ...
    controllers = {
        "posterior_discrete_selector": {
            metric: {
                "per_event_seed": dict(zip(
                    protocol.DEVELOPMENT_EVENT_SEEDS, candidate[metric])),
    ...
    selected = controllers["posterior_discrete_selector"][
        "switching_full_cycle"]["mean"]
    recovery = ((selected - robust) / (oracle - robust)
...
def render(summary: dict[str, Any]) -> str:
    ...
    lines = [
        "# Posterior-discrete fallback-fill audit",
        "",
        "| Metric | Robust | Hard CUSUM | Utility oracle | Discrete selector | "
    ...
    for metric in METRICS:
        ...
        lines.append(
            ...
            f"{controllers['posterior_discrete_selector'][metric]['mean']:.1f} | "
            f"{comparisons['learned_utility_router'][metric]['mean']:+.1f} | "
            f"{comparisons['robust'][metric]['mean']:+.1f} |")
    full_routing = summary["routing"]["switching_full_cycle"]
    lines += [
        "",
        f"- Hard fallback encountered: `{full_routing['hard_fallback_rate']:.1%}`",
        f"- Fallback replaced by specialist: "
        f"`{full_routing['fallback_replacement_rate']:.1%}`",
...
def main() -> None:
    ...
    print(
        "POSTERIOR DISCRETE ANALYSIS COMPLETE: "
