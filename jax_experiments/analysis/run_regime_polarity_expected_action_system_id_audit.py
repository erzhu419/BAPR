"""Audit expected-action inverse system ID on frozen policy seeds."""
...
from jax_experiments.analysis import (
    run_regime_polarity_inverse_system_id_audit as v3_audit,
)
...
EVENT_SCHEMA = "bapr.regime-polarity-expected-action-system-id-event.v4"
InverseSystemIDEstimator = v3_audit.InverseSystemIDEstimator
...
def _identity(seed: int) -> dict[str, Any]:
    return {
        ...
        "arms": list(ARMS),
        "benchmark_role": "development_expected_action_supervision_ablation",
        "online_inputs": [
            "observation",
            "commanded_action",
            "next_observation",
        ...
        "training_forbidden": ["realized_executed_action"],
    }
...
def _event_result(
    ...
    return {
        ...
        "training_forbidden": _identity(seed)["training_forbidden"],
...
def validate_audit(seed: int) -> dict[str, Any]:
    ...
    if (payload.get("schema") != protocol.AUDIT_SCHEMA
        ...
        raise ValueError(f"invalid expected-action audit: {destination}")
    expected = {
    ...
    if set(records) != expected:
        raise ValueError(f"incomplete expected-action audit: {destination}")
    for relative, record in records.items():
        path = destination / relative
        if not path.is_file() or protocol.file_record(path) != record:
            raise ValueError(f"expected-action audit file changed: {path}")
    for event_seed in protocol.TEST_EVENT_SEEDS:
        ...
        if (result.get("schema") != EVENT_SCHEMA
                ...
                or result.get("training_forbidden")
                != _identity(seed)["training_forbidden"]
                or {row["arm"] for row in result["switching"]}
                != set(ARMS)
                or len(result["stationary"])
                != len(ARMS) * len(protocol.MODES)):
            raise ValueError("invalid expected-action event result")
        for arm in LEARNED_ARMS:
            with np.load(
                    destination
                    / f"event_seed_{event_seed}/{arm}_trace.npz",
                ...
                if trace["posterior_before"].shape != expected_shape:
                    raise ValueError(
                        "expected-action posterior trace has wrong horizon")
                if not np.allclose(
                        np.sum(trace["posterior_before"], axis=-1),
                        1.0,
                        atol=1e-5):
                    raise ValueError(
                        "expected-action posterior is not normalized")
    return payload
...
def run(seed: int):
    ...
    if (destination / "audit_manifest.json").is_file():
        try:
            ...
            print(
                "EXPECTED-ACTION AUDIT ALREADY COMPLETE: "
                f"{destination}",
                flush=True,
            )
            return
    ...
    validate_audit(seed)
    print(f"EXPECTED-ACTION AUDIT COMPLETE: {destination}", flush=True)
