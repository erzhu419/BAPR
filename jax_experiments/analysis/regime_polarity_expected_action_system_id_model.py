"""Construction and loading for expected-action inverse system ID."""
...
def load_model(obs_dim: int, act_dim: int):
    ...
    if (manifest.get("schema") != protocol.MODEL_SCHEMA
        ...
        raise ValueError("invalid or stale expected-action system-ID model")
    ...
    if (gains.shape != expected_gains.shape
        ...
        raise ValueError("invalid expected-action calibration arrays")
