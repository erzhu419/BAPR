"""Run strict held-out audits for one paper-fidelity smoke controller."""
...
def _validate_event(
    ...
    if (len(summary) != 3
        ...
        raise ValueError("fidelity audit summary is incomplete")
    train_summary = summary[0]
    if (int(float(train_summary["n_tasks"])) != 0
            or train_summary["return_mean"].lower() != "nan"):
        raise ValueError("fidelity audit unexpectedly evaluated train tasks")
    if len(task_rows) != protocol.AUDIT_TASKS:
        raise ValueError("fidelity audit held-out task count is wrong")
    if {row["split"] for row in task_rows} != {"test"}:
        raise ValueError("fidelity audit leaked training tasks")
    if len(switching) != protocol.AUDIT_SWITCHING_EPISODES:
        raise ValueError("fidelity audit switching episodes are incomplete")
    if len(trace) != (
            protocol.AUDIT_SWITCHING_EPISODES
            * protocol.MAX_EPISODE_STEPS):
        raise ValueError("fidelity audit switching trace is incomplete")
    for row in (*summary, *task_rows, *switching, *trace):
        if (row.get("algo") != protocol.require_role(role)
                ...
                or int(float(row["seed"])) != protocol.require_training_seed(seed)
                or int(float(row["checkpoint_next_iter"]))
                != protocol.MAX_ITERS
                or int(float(row["checkpoint_total_steps"]))
                != protocol.FINAL_TOTAL_STEPS):
            raise ValueError("fidelity audit provenance is wrong")
...
def validate_audit(env: str, role: str, seed: int) -> dict:
    ...
    if (payload.get("schema") != protocol.AUDIT_SCHEMA
        ...
        raise ValueError(f"invalid fidelity audit: {directory}")
    ...
    if set(records) != expected:
        raise ValueError(f"incomplete fidelity audit: {directory}")
    for relative, record in records.items():
        path = directory / relative
        if not path.is_file() or protocol.file_record(path) != record:
            raise ValueError(f"fidelity audit file changed: {path}")
    for event_seed in protocol.EVENT_SEEDS:
        _validate_event(
...
def run(env: str, role: str, seed: int) -> None:
    ...
    if protocol.audit_manifest(env, role, seed).is_file():
        try:
            ...
            print(f"FIDELITY AUDIT ALREADY COMPLETE: {destination}")
            return
    ...
    try:
        ...
        for event_seed in protocol.EVENT_SEEDS:
            ...
            print("FIDELITY AUDIT:", " ".join(command), flush=True)
    ...
    validate_audit(env, role, seed)
    print(f"FIDELITY AUDIT COMPLETE: {destination}", flush=True)
