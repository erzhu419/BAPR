"""Train one independent source controller for the polarity oracle ladder."""
...
def training_command(role: str, seed: int) -> list[str]:
    ...
    command = [
        ...
        "--ensemble_size", str(protocol.role_ensemble_size(role)),
        "--hidden_dim", "256",
        "--lr", "0.0003",
...
def expected_config(role: str, seed: int) -> dict[str, object]:
    ...
    return {
        ...
        "context_warmup_iters": 50,
        "ensemble_size": protocol.role_ensemble_size(role),
        "hidden_dim": 256,
        "lr": 0.0003,
...
def validate_signature(role: str, seed: int) -> dict:
    ...
    if mismatches:
        raise ValueError(f"source-controller config mismatch: {mismatches}")
    return signature
...
def validate_bundle(role: str, seed: int) -> dict:
    ...
    if (payload.get("schema") != protocol.BUNDLE_SCHEMA
        ...
        raise ValueError(f"invalid source-controller bundle: {directory}")
    records = payload.get("files") or {}
    if set(records) != required:
        raise ValueError(f"incomplete source-controller bundle: {directory}")
    for relative, expected in records.items():
        path = directory / relative
        if not path.is_file() or protocol.file_record(path) != expected:
            raise ValueError(f"source-controller bundle changed: {path}")
    return payload
...
def publish_bundle(role: str, seed: int) -> dict:
    ...
    if checkpoint != protocol.expected_checkpoint(role):
        raise ValueError(f"incomplete source-controller checkpoint: {checkpoint}")
...
def run(role: str, seed: int) -> None:
    ...
    if manifest.is_file():
        ...
        print(f"SOURCE CONTROLLER ALREADY COMPLETE: {manifest}", flush=True)
        return
    command = training_command(role, seed)
    print("SOURCE CONTROLLER TRAIN:", " ".join(command), flush=True)
    ...
    if replay.is_file():
        replay.unlink()
    print("SOURCE CONTROLLER COMPLETE: " + json.dumps(
