"""Run strict held-out audits for one RE-SAC artifact confirmation run."""
...
def _validate_event(
    ...
    if (len(summary) != 3
        ...
        raise ValueError("artifact-confirmation audit summary is incomplete")
    train_summary = summary[0]
    if (int(float(train_summary["n_tasks"])) != 0
            or train_summary["return_mean"].lower() != "nan"):
        raise ValueError("artifact audit unexpectedly evaluated train tasks")
    if len(task_rows) != protocol.AUDIT_TASKS:
        raise ValueError("artifact audit held-out task count is wrong")
    if {row["split"] for row in task_rows} != {"test"}:
        raise ValueError("artifact audit leaked training tasks")
    if len(switching) != protocol.AUDIT_SWITCHING_EPISODES:
        raise ValueError("artifact audit switching episodes are incomplete")
    if len(trace) != (
            protocol.AUDIT_SWITCHING_EPISODES
            * protocol.MAX_EPISODE_STEPS):
        raise ValueError("artifact audit switching trace is incomplete")
    for row in (*summary, *task_rows, *switching, *trace):
        if (row.get("algo") != protocol.require_role(role)
            ...
            raise ValueError("artifact audit provenance is wrong")


def audit_identity(env: str, role: str, seed: int) -> dict[str, object]:
    return {
        **protocol.identity(env, role, seed),
        ...
        "evaluation_split": "strict_heldout_validation",
    }
...
def validate_audit(env: str, role: str, seed: int) -> dict:
    ...
    if (payload.get("schema") != protocol.AUDIT_SCHEMA
            ...
            or payload.get("identity") != audit_identity(env, role, seed)):
        raise ValueError(f"invalid artifact audit: {directory}")
    ...
    if set(records) != expected:
        raise ValueError(f"incomplete artifact audit: {directory}")
    for relative, record in records.items():
        path = directory / relative
        if not path.is_file() or protocol.file_record(path) != record:
            raise ValueError(f"artifact audit file changed: {path}")
    for event_seed in protocol.EVENT_SEEDS:
        _validate_event(
...
def run(env: str, role: str, seed: int) -> None:
    ...
    if protocol.audit_manifest(env, role, seed).is_file():
        try:
            ...
            print(f"ARTIFACT AUDIT ALREADY COMPLETE: {destination}")
            return
    ...
    try:
        ...
        for event_seed in protocol.EVENT_SEEDS:
            ...
            command = audit_command(env, role, seed, event_seed, event_dir)
            commands.append(command)
            print("ARTIFACT AUDIT:", " ".join(command), flush=True)
        ...
        protocol.write_json_atomic(temporary / "audit_manifest.json", {
            ...
            "status": "complete",
            "identity": audit_identity(env, role, seed),
    ...
    validate_audit(env, role, seed)
    print(f"ARTIFACT AUDIT COMPLETE: {destination}", flush=True)
