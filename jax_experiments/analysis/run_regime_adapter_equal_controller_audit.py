"""Run one strict audit of the full-budget independent adapter bank."""
...
from jax_experiments.analysis.run_regime_adapter_equal_controller_branch import (
...
CASES = (
    ...
    development.EvaluationCase(
        "identity_adapter", "bank", protocol.MODES),
    *(development.EvaluationCase(
...
def _training_manifest_paths(seed: int) -> tuple[Path, ...]:
    return (
        protocol.robust_bundle_dir(seed)
...
def _expected_aggregate_steps(case: development.EvaluationCase) -> int:
    if case.kind == "robust":
        return protocol.FINAL_TOTAL_STEPS
    return protocol.BANK_AGGREGATE_TOTAL_STEPS


def _validate_case(
        directory: Path, seed: int, event_seed: int,
        case: development.EvaluationCase) -> None:
    expected_counts = {
        "summary.csv": 3,
    ...
    for filename, expected_count in expected_counts.items():
        ...
        for row in rows:
            if (row.get("controller_case") != case.label
                    ...
                    != _expected_aggregate_steps(case)
...
def validate_audit(seed: int, event_seed: int) -> dict:
    ...
    expected_identity = {
        **protocol.identity(seed, protocol.MODES[0]),
        "mode": "all",
    ...
    if (payload.get("schema") != protocol.AUDIT_SCHEMA
        ...
        raise ValueError(f"invalid equal-controller audit: {destination}")
    for relative, expected in payload["files"].items():
        path = destination / relative
        if not path.is_file() or protocol.file_record(path) != expected:
            raise ValueError(f"equal-controller audit file changed: {path}")
    for case in CASES:
        _validate_case(
...
def _evaluate(
        seed: int, event_seed: int, case: development.EvaluationCase,
    ...
    if case.kind == "robust":
        ...
        aggregate_steps = protocol.FINAL_TOTAL_STEPS
    else:
        ...
        algo_label = "regime_adapter_equal_controller_bank"
        aggregate_steps = protocol.BANK_AGGREGATE_TOTAL_STEPS
    ...
    try:
        ...
        meta = {
            ...
            "checkpoint_next_iter": protocol.FINAL_NEXT_ITERATION,
            "per_controller_steps": protocol.FINAL_TOTAL_STEPS,
            "aggregate_controller_steps": aggregate_steps,
            "compute_matched": False,
            "budget_semantics": (
                "Diagnostic upper bound: each fixed-mode adapter receives "
                "the full 2.8M post-fork transitions used by the robust "
                "continuation; the four-adapter bank uses 4x post-fork data."
            ),
...
def run(seed: int, event_seed: int) -> None:
    ...
    if manifest.is_file():
        ...
        print(f"EQUAL-CONTROLLER AUDIT ALREADY COMPLETE: {destination}")
        return
    ...
    try:
        ...
        for case in CASES:
            ...
            print(
                f"EQUAL-CONTROLLER AUDIT seed={seed} "
    ...
    validate_audit(seed, event_seed)
    print(f"EQUAL-CONTROLLER AUDIT COMPLETE: {destination}", flush=True)
