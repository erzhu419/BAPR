"""V20-runner compatibility view for the V32 canonical reference."""
...
def run_dir(variant: str, seed: int, mode: int):
    ...
    if require_mode(mode) != final.REFERENCE_MODE:
        raise ValueError("V32 trains only the preregistered canonical mode")
...
def bundle_dir(variant: str, seed: int, mode: int):
    require_variant(variant)
    if require_mode(mode) != final.REFERENCE_MODE:
        raise ValueError("V32 bundles only the preregistered canonical mode")
...
def bundle_required_paths(variant: str, seed: int, mode: int):
    require_variant(variant)
    if require_mode(mode) != final.REFERENCE_MODE:
        raise ValueError("V32 requires only the canonical reference bundle")
...
def identity(variant: str, seed: int, mode: int):
    require_variant(variant)
    if require_mode(mode) != final.REFERENCE_MODE:
        raise ValueError("V32 identity is fixed to canonical mode 0")
