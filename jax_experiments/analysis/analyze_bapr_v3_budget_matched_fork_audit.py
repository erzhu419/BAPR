...
CATEGORICAL_AUDIT_VALIDATOR_SHA256 = frozenset({
    "d7607ee69d813c9904e85ad05ee8fa20dfd66c36b7e87473f0e42ec252f0a426",
    "1779008896b1b49d747e85192e8059fa7dcca9a00c513a8175f86bf27877185a",
})
CATEGORICAL_AUDIT_VALIDATOR_LEGACY_SHA256 = frozenset({
    "d7607ee69d813c9904e85ad05ee8fa20dfd66c36b7e87473f0e42ec252f0a426",
})
...
def expected_orchestrator_source_provenance(
    ...
    live_validator_hash = live[AUDIT_VALIDATOR_MODULE]
    if live_validator_hash not in CATEGORICAL_AUDIT_VALIDATOR_SHA256:
        raise ValueError(
            "unrecognized categorical audit-validator overlay hash: "
            f"{live_validator_hash}")
    if live_validator_hash in CATEGORICAL_AUDIT_VALIDATOR_LEGACY_SHA256:
        authorization = dict(authorization)
        authorization.pop("finalization_validator_sha256")
    return {
...
def validate_recorded_orchestrator_source_provenance(
    recorded: dict[str, Any],
    source: dict[str, Any],
    pair_manifest: dict[str, Any],
) -> None:
    """Validate a group against either preregistered overlay generator."""
    producer, _live = _audit_orchestrator_hashes(source)
    if recorded == producer:
        return

    require_equal(
        recorded.get("mode"), CATEGORICAL_VALIDATOR_OVERLAY_MODE,
        "group validator-overlay mode")
    require_equal(
        require_dict(recorded.get("producer"), "group overlay producer"),
        producer,
        "group overlay producer hashes")
    require_equal(
        recorded.get("changed_files"), [AUDIT_VALIDATOR_MODULE],
        "group overlay changed modules")
    recorded_live = require_dict(
        recorded.get("live"), "group overlay live hashes")
    require_equal(
        set(recorded_live), set(AUDIT_ORCHESTRATOR_MODULES),
        "group overlay live module names")
    for relative in AUDIT_ORCHESTRATOR_MODULES:
        digest = require_sha256(
            recorded_live.get(relative),
            f"group overlay live hash {relative}")
        if relative != AUDIT_VALIDATOR_MODULE:
            require_equal(
                digest, producer[relative],
                f"group overlay producer-exact hash {relative}")

    validator_hash = recorded_live[AUDIT_VALIDATOR_MODULE]
    if validator_hash not in CATEGORICAL_AUDIT_VALIDATOR_SHA256:
        raise ValueError(
            "unrecognized recorded categorical audit-validator hash: "
            f"{validator_hash}")
    expected_authorization = _categorical_overlay_authorization(
        source, pair_manifest)
    if validator_hash in CATEGORICAL_AUDIT_VALIDATOR_LEGACY_SHA256:
        expected_authorization = dict(expected_authorization)
        expected_authorization.pop("finalization_validator_sha256")
    require_equal(
        require_dict(
            recorded.get("authorization"),
            "group overlay authorization"),
...
def validate_group_bundle(
    ...
    validate_recorded_orchestrator_source_provenance(
        orchestrator, pair_source, pair_manifest)
