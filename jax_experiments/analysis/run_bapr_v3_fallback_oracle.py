"""Run the privileged true-mode fallback-oracle full-cycle audit."""
...
from jax_experiments.analysis import (
    bapr_v3_fallback_oracle as protocol,
)
from jax_experiments.analysis import (
...
def evaluate_full_cycle(
    ...
    for episode in range(base_audit.SWITCHING_EPISODES):
        sequence = base_audit._configure_switching(
            env, tasks, "full_cycle", episode)
        base_key = (
            20260718 + event_seed * 100_000
            + 500_000 + episode * 10_000)
        ...
        for step in range(horizon):
            ...
            hard_selected, diagnostics = router.decision()
            fallback = bool(diagnostics["fallback"])
            selected = protocol.select_controller(
                hard_selected, fallback, physics_mode, oracle_map)
            action = _policy_action(
                action_fn, policy_states, selected, observation)
            env.rng = jax.random.PRNGKey(base_key + step * 2 + 1)
            next_observation, reward, done, info = env.step(action)
            if int(info["mode_used"]) != physics_mode:
                raise RuntimeError("fallback-oracle action used wrong mode")
            router.observe(
                observation, action, reward, next_observation, done)
            decisions.append(selected)
            physics_modes.append(physics_mode)
            hard_fallback.append(fallback)
            specialist_replacements.append(
                fallback
                and selected != protocol.discrete.prior.utility.ROBUST_CONTROLLER)
        ...
        route = base_audit._routing_summary(
            decisions, physics_modes, oracle_map)
        route.update({
            "episode": episode,
            "hard_fallback_rate": float(np.mean(hard_fallback)),
            "fallback_specialist_replacement_rate": float(
                np.mean(specialist_replacements)),
        })
...
def validate_group(payload: dict[str, Any]) -> None:
    ...
    if (payload.get("schema") != protocol.GROUP_SCHEMA
        ...
        raise ValueError("invalid fallback-oracle audit identity")
    reference = protocol.discrete.prior.utility.audit_group_path(
        "validation", event_seed, protocol.DECISION_VARIANT)
    if payload.get("reference_baseline_file") != protocol.file_record(reference):
        raise ValueError("fallback-oracle baseline reference changed")
    record = payload.get("switching_full_cycle") or {}
    episodes = record.get("episodes") or []
    if (len(episodes) != base_audit.SWITCHING_EPISODES
            or not all(math.isfinite(float(row["return"]))
                       for row in episodes)):
        raise ValueError("invalid fallback-oracle episodes")
...
def run(event_seed: int) -> Path:
    ...
    if result_path.is_file():
        ...
        print(f"Complete fallback-oracle audit exists: {output}")
        return result_path

    table = protocol.discrete.prior.utility.load_utility_table()
    ...
    bundles = (
        protocol.discrete.prior.utility.control.specialist_protocol
    ...
    source_before = (
        protocol.discrete.prior.utility.control.fork_protocol
    ...
    router_config = protocol.discrete.prior.utility.decision_config(
    ...
    if source_after["sha256"] != source_before["sha256"]:
        raise RuntimeError("source changed during fallback-oracle audit")
    ...
    payload = {
        ...
        "event_seed_role": "privileged fallback capacity stream",
        "decision_variant": protocol.DECISION_VARIANT,
        "selection_rule": (
            "preserve committed hard-CUSUM controller; on fallback use the "
            "true-mode utility-oracle controller"),
        ...
        "utility_table_file": protocol.file_record(
            protocol.discrete.prior.utility.TABLE_PATH),
        ...
        "switching_full_cycle": switching,
    }
    validate_group(payload)
    ...
    print(f"FALLBACK ORACLE AUDIT COMPLETE: {output}", flush=True)
    return result_path
