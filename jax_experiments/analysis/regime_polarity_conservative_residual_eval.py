"""Bind strict anchored evaluation to a conservative residual variant."""
...
from jax_experiments.analysis import (
    regime_polarity_conservative_residual as protocol_module,
)
from jax_experiments.analysis import (
    regime_polarity_frozen_anchor as v2_protocol,
)
from jax_experiments.analysis import (
    run_regime_polarity_conservative_residual_branch as branch_runner,
)
from jax_experiments.analysis import (
    run_regime_polarity_frozen_anchor_branch as v2_branch_runner,
)
...
def load_controller(role: str, seed: int):
    ...
    if role == "robust_continue":
        actual_role = "robust_long"
        v2_branch_runner.protocol = v2_protocol
        v2_branch_runner.validate_published(seed, actual_role)
        directory = v2_protocol.branch_bundle_dir(actual_role, seed)
        expected_algo = v2_protocol.branch_algo(actual_role)
    else:
        actual_role = view.variant
    ...
    if (
        ...
        raise ValueError(
            f"wrong conservative residual controller config: {directory}")
    ...
    if (
        ...
        raise ValueError(f"stale conservative controller: {directory}")
